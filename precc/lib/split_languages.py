...
for lc in cld2_langcodes:
    filename = os.path.join(args.outdir, "text.{}.xz".format(lc))
    lang2file[lc] = lzma.open(filename, 'w')
...
for line in sys.stdin:
    if line.startswith(magic_number):
        if buf:
            ...
            lang2file[current_lang].write("".join(buf).encode("utf-8"))
...
for langfile in lang2file.values():
    langfile.close()
